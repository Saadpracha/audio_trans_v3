...
def webhook3_listener(payload: dict):
    ...
    - Supports: contact.website, contact.company_name
    - caller_name can be derived from phoneCall.user.name if missing
    ...
    try:
        ...
        if isinstance(phone_call, dict):
            user_obj = phone_call.get("user") if isinstance(phone_call.get("user"), dict) else None
            if user_obj and user_obj.get("name") and not payload.get("caller_name"):
                payload["caller_name"] = user_obj.get("name")
    except Exception:
        pass
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_whisper_service_3:app", host="0.0.0.0", port=int(os.getenv("PORT", 8088)), log_level="info")
